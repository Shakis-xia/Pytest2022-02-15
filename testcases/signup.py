...
@pytest.mark.parametrize('caseinfo,url,method,type,args,expect_res,check_sql,exptce_db_res',cases)
def test_login(caseinfo,url,method,type,args,expect_res,check_sql,exptce_db_res):
    res_type,real_res,s = send_requests(url,method,type,args)
    passed,msg = check(caseinfo,expect_res,res_type,real_res)
    assert passed,msg
    db_passed,db_msg = DB().check_db(caseinfo,check_sql,exptce_db_res)
    assert db_passed,db_msg
